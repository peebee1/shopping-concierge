...
def recommend_endpoint(req: RecommendRequest) -> dict:
    result = recommend(req.query, _products, _llm, top_n=req.top)
...
f.onsubmit=async e=>{e.preventDefault();
 ...
 let rows=(d.ranked||[]).map(x=>`<tr><td>${x.rank}</td><td><b>${x.name}</b><br><small>${x.brand} · ${x.category}</small></td><td>$${x.price}</td><td>${x.rating}</td><td>${x.rationale}</td></tr>`).join('');
 out.innerHTML=`<div class="summary"><b>${d.summary}</b></div><table><tr><th>#</th><th>Product</th><th>Price</th><th>Rating</th><th>Why</th></tr>${rows}</table><div class="trace">${(d.trace||[]).map((t,i)=>`${i+1}. ${t}`).join('\\n')}</div>`;
};
</script></body></html>"""
